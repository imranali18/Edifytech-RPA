...
// 20190124T181700 Konstantin Kuzmin - the whole operator had to be changed to fix
// CS 1 Lecture Exercises drop lowest 3 problem
// Previously, when using this operator (implemented as follows:
//---------------------
//bool operator<(const score_object &a, const score_object &b) {
//  return a.score < b.score;
//}
//---------------------
//) the lowest scores were dropped regardless of the max and the percentage. It was causing the
// lowest absolute values of scores to always be dropped.
// E.g., among five scores 8/8, 8/8, 10/12, 15/30, 12/50, it would select 8/8, 8/8, and 10/12
// to drop although these are the highest scores in this list if max is taken into account.
bool operator<(const score_object &a, const score_object &b) {
  float s1 = a.score;
  float m1 = a.max;
...
float Student::GradeablePercent(GRADEABLE_ENUM g) const {
  ...
  for (int i = 0; i < GRADEABLES[g].getCount(); i++) {
    ...
    // Kuzmin-DEBUG
    //if (getUserName() == "kuzmik2" && g == GRADEABLE_ENUM::LECTURE_EXERCISE) std::cout << "^^^ " << s << " " << m << " " << p << " " << sm << " " << GRADEABLES[g].getID(i) << std::endl;
  ...
  for (int i = GRADEABLES[g].getRemoveLowest(); i < GRADEABLES[g].getCount(); i++) {
    ...
    sum_max += m;
  // Kuzmin-DEBUG
  //if (getUserName() == "kuzmik2" && g == GRADEABLE_ENUM::LECTURE_EXERCISE) std::cout << "@@@ " << m << " " << sum_max << std::endl;
  }
  // Kuzmin-DEBUG
  //if (getUserName() == "kuzmik2" && g == GRADEABLE_ENUM::LECTURE_EXERCISE) std::cout << getLastName() << " " << gradeable_to_string(g) << " " << GRADEABLES[g].getCount() << " " << sum_max << std::endl;
  ...
  for (int i = GRADEABLES[g].getRemoveLowest(); i < GRADEABLES[g].getCount(); i++) {
    ...
    sum += p * s / my_max;
    // Kuzmin-DEBUG
    //if (getUserName() == "kuzmik2" && g == GRADEABLE_ENUM::LECTURE_EXERCISE) std::cout << "!!! " << s << " " << m << " " << p << " " << sm << " " << my_max << " " << p * s / my_max << " " << sum << std::endl;
  }

  // Kuzmin-DEBUG
  //if (getUserName() == "kuzmik2" && g == GRADEABLE_ENUM::LECTURE_EXERCISE) std::cout << "### " << sum << " " << sum_scaled_max << " " << 100 * GRADEABLES[g].getPercent() * sum << std::endl;
